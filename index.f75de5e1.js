...
tbody.addEventListener("click", (e)=>{
    ...
    [
        ...
        if (tr !== x) x.classList.remove("active");
    });
...
const isValidValue = ({ name: nameForm, position, age, salary })=>{
    ...
    for (const v of validations){
        if (v.rule) return pushNotification(150, 10, `Invalid ${v.name}`, v.msg, "error");
    }
    pushNotification(10, 10, "New employee", "Employee added to the table", "success");
};
const addEmployee = (dataForm)=>{
    ...
    [
        ...
        if (x.nodeName !== "BUTTON") {
            ...
            td.textContent = x.name === "salary" ? `$${Number(x.value).toLocaleString("en-US")}` : x.value;
            tr.append(td);
        }
    });
    tbody.append(tr);
